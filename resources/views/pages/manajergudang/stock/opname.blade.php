...
@section('title', 'Rekonsiliasi Kuota')
...
<div class="container p-4 mx-auto sm:p-8" x-data="{ showFilter: false }">
    <div class="py-8">
        <div class="flex items-center gap-4 mb-8">
            ...
            <div>
                <h1 class="text-3xl font-bold text-gray-800 dark:text-white">Rekonsiliasi Kuota</h1>
                <p class="mt-1 text-gray-500 dark:text-gray-400">Sesuaikan kuota fisik dengan data sistem.</p>
            </div>
        </div>
        <form id="filterForm" action="{{ route('manajergudang.stock.opname') }}" method="GET" class="mb-6">
            <div class="flex flex-col gap-4 md:flex-row">
                <div class="relative flex-1">
                    ...
                    <input type="search" name="search" value="{{ request('search') }}" placeholder="Cari jamaah/paket berdasarkan nama atau SKU..." class="w-full px-4 py-3 pl-12 pr-4 border rounded-lg dark:bg-slate-700 dark:border-gray-600 dark:text-white focus:ring-2 focus:ring-blue-500">
                </div>
            ...
            <div class="overflow-hidden bg-white rounded-xl shadow-lg dark:bg-slate-800">
                <div class="overflow-x-auto">
                    <table class="w-full table-fixed">
                        <thead class="bg-gray-50 dark:bg-slate-700/50"><tr><th class="w-2/5 px-4 py-4 text-xs font-semibold tracking-wider text-left text-gray-400 uppercase"><i class="mr-2 fas fa-user-friends"></i>Jamaah / Paket</th><th class="w-1/4 px-4 py-4 text-xs font-semibold tracking-wider text-left text-gray-400 uppercase"><i class="mr-2 fas fa-truck"></i>Mitra/Agen (Opsional)</th><th class="w-1/6 px-4 py-4 text-xs font-semibold tracking-wider text-center text-gray-400 uppercase"><i class="mr-2 fas fa-clipboard-check"></i>Kuota Sistem</th><th class="w-1/6 px-4 py-4 text-xs font-semibold tracking-wider text-center text-gray-400 uppercase"><i class="mr-2 fas fa-ruler-combined"></i>Kuota Fisik</th><th class="w-1/6 px-4 py-4 text-xs font-semibold tracking-wider text-center text-gray-400 uppercase"><i class="mr-2 fas fa-arrows-alt-h"></i>Selisih</th></tr></thead>
                        <tbody>
                            @forelse($products as $product)
                                <tr class="border-b dark:border-slate-700 last:border-b-0" x-data="{ systemStock: {{ $product->current_stock ?? 0 }}, physicalStock: {{ old('products.'.$loop->index.'.physical_stock', $product->current_stock ?? 0) }} }">
                                    ...
                                    <td class="p-4"><select name="products[{{ $loop->index }}][supplier_id]" class="w-full px-4 py-2 text-sm border rounded-lg dark:bg-slate-700 dark:border-gray-600 dark:text-white"><option value="">-- Tanpa Mitra --</option>@foreach($suppliers as $supplier)<option value="{{ $supplier->id }}" {{ $product->supplier_id == $supplier->id ? 'selected' : '' }}>{{ $supplier->name }}</option>@endforeach</select></td>
                            ...
                            @empty
                                <tr><td colspan="5" class="px-6 py-16 text-center text-gray-400"><div class="flex flex-col items-center"><i class="mb-4 text-5xl fas fa-box-open opacity-50"></i><p class="text-lg">Data jamaah/paket tidak ditemukan</p><p class="text-sm">Coba reset filter atau gunakan kata kunci lain.</p></div></td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
                @if($products->hasPages())<div class="p-4 border-t dark:border-slate-700">{{ $products->appends(request()->query())->links() }}</div>@endif
                <div class="flex items-center justify-end p-6 border-t dark:border-slate-700"><button type="submit" class="px-8 py-3 font-semibold text-white bg-blue-600 rounded-lg shadow-lg hover:bg-blue-700" onclick="return confirm('Apakah Anda yakin ingin menyimpan hasil rekonsiliasi? Kuota akan disesuaikan secara permanen.')"><i class="mr-2 fas fa-save"></i>Simpan & Sesuaikan Kuota</button></div>
            </div>
        </form>
    </div>
</div>
