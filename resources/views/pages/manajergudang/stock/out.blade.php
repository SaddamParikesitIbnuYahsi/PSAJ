...
@section('title', 'Catat Keberangkatan')
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('stockForm', () => ({
                ...
                const tomselect = new TomSelect(selectEl, {
                    ...
                    render: {
                        option: function(data, escape) {
                            return `<div class="flex items-center space-x-3"><img class="w-10 h-10 object-cover rounded" src="${escape(data.image_url)}" alt="${escape(data.text)}"><div><div class="font-medium">${escape(data.text)}</div><div class="text-xs text-gray-500">Kuota: ${escape(data.stock)} ${escape(data.unit)}</div></div></div>`;
                        },
            ...
            validateQuantity() {
                if (this.quantity > this.currentStock) { alert('Jumlah keberangkatan tidak boleh melebihi kuota saat ini!'); this.quantity = this.currentStock; }
    ...
    <div x-data="stockForm" x-init="init()">
        <div class="py-8">
            ...
            <div class="flex items-center gap-4 mb-8">
                ...
                <div>
                    <h1 class="text-3xl font-bold text-gray-800 dark:text-white">Catat Keberangkatan</h1>
                    <p class="mt-1 text-gray-500 dark:text-gray-400">Catat jamaah yang berangkat (kurangi kuota seat).</p>
                </div>
            </div>
            <form action="{{ route('manajergudang.stock.out.store') }}" method="POST" class="grid grid-cols-1 gap-8 mt-8 lg:grid-cols-3">
                ...
                <div class="p-6 space-y-6 bg-white rounded-xl shadow-lg lg:col-span-2 dark:bg-slate-800">
                    ...
                    @endif
                    <div><label for="product_id" class="flex items-center mb-2 text-sm font-medium text-gray-700 dark:text-gray-300"><i class="w-5 mr-2 fas fa-user-friends"></i>Jamaah / Paket <span class="ml-1 text-red-500">*</span></label><select id="product_id" name="product_id" placeholder="Cari dan pilih jamaah/paket..." required>@foreach ($products as $product)<option value="{{ $product->id }}" data-image_url="{{ $product->image ? asset('storage/' . $product->image) : 'https://ui-avatars.com/api/?name='.urlencode($product->name) }}" data-stock="{{ $product->current_stock }}" data-unit="{{ $product->unit }}" {{ old('product_id', request()->get('product_id')) == $product->id ? 'selected' : '' }}>{{ $product->name }}</option>@endforeach</select></div>
                ...
                <div class="lg:col-span-1">
                    <div class="sticky p-6 bg-white rounded-xl shadow-lg top-24 dark:bg-slate-800">
                        <div x-show="!selectedProductId" class="py-10 text-center text-gray-500 dark:text-gray-400"><i class="mb-4 text-5xl fas fa-box-open opacity-50"></i><p>Pilih jamaah/paket untuk melihat informasi kuota.</p></div>
                        <div x-show="selectedProductId" x-cloak class="space-y-4">
                            <h3 class="flex items-center pb-4 text-lg font-semibold text-gray-900 border-b dark:text-white dark:border-slate-700"><i class="mr-3 text-blue-500 fas fa-info-circle"></i>Informasi Kuota</h3>
                            ...
                            <div class="pt-4 space-y-2 border-t dark:border-slate-700">
                                <div class="flex items-center justify-between"><span class="text-sm text-gray-600 dark:text-gray-400">Kuota Saat Ini:</span><span class="text-lg font-bold text-gray-900 dark:text-white" x-text="`${currentStock} ${unit}`"></span></div>
                                <div class="flex items-center justify-between text-sm"><span class="text-gray-600 dark:text-gray-400">Jumlah Keberangkatan:</span><span class="font-semibold text-red-500" x-text="`- ${quantity || 0} ${unit}`"></span></div>
                            </div>
                            <div class="pt-4 border-t dark:border-slate-700">
                                <div class="flex items-center justify-between"><span class="font-medium text-gray-600 dark:text-gray-400">Kuota Setelahnya:</span><span class="text-2xl font-bold" :class="finalStock < 0 ? 'text-red-500' : 'text-blue-600 dark:text-blue-400'" x-text="`${finalStock} ${unit}`"></span></div>
                            </div>
                            <div class="pt-6"><button type="submit" class="flex items-center justify-center w-full px-6 py-3 font-semibold text-white bg-blue-600 rounded-lg shadow-md hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed" :disabled="finalStock < 0 || quantity < 1"><i class="mr-2 fas fa-save"></i>Simpan Transaksi</button><p x-show="finalStock < 0" class="mt-2 text-xs text-center text-red-500">Kuota tidak boleh negatif!</p></div>
                        </div>
                    </div>
                </div>
            </form>
